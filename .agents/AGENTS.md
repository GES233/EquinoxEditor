...
- **Pipes and Missing Utilities**: When using pipe operators (`|`) in the `mvdan/sh` bash emulator on Windows, standard Unix text-processing utilities (like `tail`, `head`, `awk`, `grep`) are often missing from the `$PATH` (e.g., `"tail": executable file not found in $PATH`). The shell parses the pipe syntax successfully, but the external commands are not available on the Windows host. To avoid errors, prefer using Crush's native tools (like `View`, `Grep`, `Glob`) instead of relying on Unix CLI commands through pipes.
