...
This is **not** a mix umbrella: the root `mix.exs` is a bare placeholder (`:equinox_repo`, no deps). The real projects are four sibling Elixir projects wired by path dependencies:
...
- `engine_adapters/` — `:equinox_engine_adapters` (userland engine adapter reference implementations `EquinoxAdapters.*`: `Stub` / `UTAUCompat` / `UTAUDiffSingerCompat` / `DiffSinger.*`; path-depends on domain + kernel; ui_shell depends on it for the demo session's `default_engine` Stub wiring; includes the DiffSinger Python sidecar (`sidecar/`, uv-managed) and real-inference Step)
...
- Engine Adapters (`cd engine_adapters`): `mix deps.get`, `mix test`, `mix precommit`
...
20. **RenderRequest + AdoptRequest** — main wiring done: `prepare_dispatch/1` builds one `RenderRequest` per window via `from_window/3`; `Runner.run/3` is two-phase (check-all → render-all), resolving patches per channel via `Tamale.Patch.resolve/2` against the Configurator `channels` contract (`projection` arity-2 + `target`, PortRef-keyed); check failures aggregate as `{:error, {:check_failed, entries}}`; `Server.adopt_intervention/3` wraps `AdoptRequest.build_patch/3` + `Command.attach_patches`. Channel developer guide: `docs/channel-development.md`. Second cut landed (2026-08-15): curve channel + frame-grid `timing_spec` consumption + rasterization (see below).
...
**Engine Adapter (2026-08-15 boundary decision; first cut landed)**: the headless-editor role is kernel-owned; coconut's `Render.Engine`/`Resolve`/`Encoder` stack is dormant (marked, not deleted, not adopted). `Equinox.Kernel.EngineAdapter` behaviour is defined with five callbacks (`channels` / `engine_key` / `timing_spec` / `globals` / `adoptables`); `Configurator.new(engine: {adapter, config})` derives channel specs from it (single source, derived wins over hand-injected `:channels`). Per-track granularity: `Session.Context.engines` (a `%{voicebank_id => {adapter, config}}` registry injected via `Server` start opts) + `TrackMeta.voicebank_id` (set via `Server.update_track_voicebank/3`) resolve each track's adapter in `prepare_dispatch`, which hangs derived channel specs on `dispatch.track_channels`; `Runner` looks up specs per unit (fallback `Configurator.channels`). Engine version stamps enter digest bases via `Channel.stamp_base/2` + `AdoptRequest`'s `:engine` option (mount side) and adapter spec projections (check side) — `Server.adopt_intervention/3` stamps automatically. **Globals gate (decided 2026-08-15)**: knob values live in `TrackMeta.globals` (`Server.update_track_globals/3`, key-merge with nil-deletes, not undoable); `prepare_dispatch` hangs values on `dispatch.track_globals` and per-track rules (derived from `adapter.globals/1`) on `track_global_rules` (fallback `Configurator.global_rules`; nil = no declaration = no gate); the Runner check phase validates them with one-vote aggregation, entries `%{kind: :global, track_id, key, reason}` joining the same `check_failed` list as patch conflicts. **Capabilities gating (decided 2026-08-15)**: kernel-owned, adapter-declared — check side stays `:unknown_channel`; adopt side, `Server.adopt_intervention/3` checks the channel atom (module `channel/0`) against the track adapter's `adoptables/1` and fails loud with `{:error, {:not_adoptable, channel}}` (ungated when the track has no adapter). **Voicebank descriptor (landed 2026-08-15)**: `Equinox.Kernel.Voicebank` — engine-side asset VO (`id` / `engine` / `engine_version` required; `models` / `dictionary` opaque maps; `capabilities.supported_channels` / `supported_params` atom lists; `timing.frame_rate` / `hop` positive integers; own dump/load). Not a domain fact: no Domain struct, no History, no undo. Mounted via adapter config (`%{voicebank: vb}`); `engine_key/1` is the `"id@engine_version"` stamp convention; the stub adapter demonstrates deriving channels/timing/engine_key from the descriptor (descriptor wins over flat config keys). Discovery/registration of voicebank assets stays a userland runtime duty. A stub adapter E2E (edit → adopt → check → render, version-upgrade conflict storm, globals gate, capabilities gating, voicebank-descriptor config, curve rasterization) is green in `kernel/test/equinox/engine_adapter_test.exs`. **MCPAdapter (landed 2026-08-15, first real adapter proof)**: `Equinox.Kernel.MCPAdapter` pulls voicebank descriptors from an external engine process over MCP stdio — capability-declaration fetch only (`fetch/1` does handshake + `resources/read` once at registry-build time; the five callbacks stay pure on the enriched config). The MCP client is hand-rolled (`Equinox.Kernel.MCP.StdioClient`, line-delimited JSON-RPC over a Port, zero new deps; anubis/hermes rejected — LGPL/weight/stalled). Channel specs for known channels are built by the shared `Equinox.Kernel.ChannelSpecs.build/3` helper (phoneme_timing/curve; the stub keeps its inline copy as a reference). **Real adapters (landed 2026-08-15)**: `EquinoxAdapters.UTAUCompat` (UTAU CV, concatenative — no frame grid, `timing_spec/1` → `{:error, :no_frame_grid}`, curve passthrough mode, resampler flags as globals, adoptables `[]`) and `EquinoxAdapters.UTAUDiffSingerCompat` (OpenUtau-style DiffSinger — frame grid `sample_rate / hop_size` as a float, `:phoneme_timing` + `:curve` rasterized, key_shift/speaker globals, both channels adoptable). Their pair pinned D1–D6 (optional timing_spec / content-hash engine_version / float frame_rate / globals rule algebra / adoptables semantics / descriptor adequacy) — see `docs/engine-adapter-design.md` "真实 adapter 边界定案". Artifact shape pinned (2026-08-15): `%{path, sample_rate, frames}` on the blackboard, wav written by the sidecar (see `docs/engine-adapter-design.md` "DiffSinger 真模型闭环"). Open detail: voicebank discovery/registration. **DiffSinger real-model loop (landed 2026-08-15)**: `engine_adapters/sidecar/` (Python MCP-stdio sidecar, 8-session ONNX pipeline ported from zongzi-svs) + `EquinoxAdapters.DiffSinger.{Sidecar, Packaging, InferStep}`; `UTAUDiffSingerCompat.channels/1` overrides the `:phoneme_timing` spec target with an arity-2 fan-out that packages the whole window (notes+spans+tempo -> sidecar words, phonemes from `note.metadata["phonemes"]`) onto `{:port, :infer, :words}`; InferStep renders via the sidecar and emits the artifact `%{path, sample_rate, frames, lead_in_sec}`. Vowel-anchored alignment (OpenUtau-style preutterance: head SP padding + unstretched backtimed head consonants) landed via the sidecar `align` tool; aligned `ph_dur` replays through render's `ph_dur_override`; `:phoneme_timing` payload semantics pinned to absolute boundaries (`docs/phoneme-alignment-research.md`). Zero kernel changes; e2e green (`@tag :real_engine`, excluded by default).
...
22b. **Intervention UI minimal loop (landed 2026-08-16)**: patches are now UI-visible — presenter projects alive patches to `TrackData.patches` (`%{id, channel, anchor: %{kind, refs?}, payload}`, payload pass-through); kernel `Session.Context.notifications` + `Server.take_notifications/1` is the UI-bound drain for dead patches (`run_history` enqueues `{:dead_patches, [...]}` alongside the log); LiveView pushes `patch_conflicts` events (dead-patch entries + adopt failures); PianoRoll renders `:curve` payloads (`CurveLayer.svelte` SVG polyline over the pitch axis, `curve_projection.ts` pure segment-relative mapping) and shows a patch chip list. **Real creation (2026-08-16, demo mocks removed)**: hand-drawn strokes — CurveLayer draw mode samples pointer strokes → `stroke.ts` (`strokeToPoints` min-gap decimation, Douglas-Peucker still deferred; `noteIdsInRange` anchoring) → `adopt_curve` event → real `Server.adopt_intervention/3`. **Engine wiring (2026-08-16)**: demo session injects `default_engine: {EquinoxAdapters.Stub, ...}` (ui_shell path-depends on engine_adapters; `Session.child_spec/1` now forwards `:engines`/`:default_engine` opts — was silently dropping them), so version stamps, adoptables gating, and check-phase spec resolution are all live in the UI session. **Grid (2026-08-16)**: `grid.ts` (`snapTick`, GRID_OPTIONS at tpqn=480) + `stores/grid.svelte.ts`; snapping applies only to notes created/modified while enabled (create / drag / resize), never retroactive; floats are rounded even when disabled. Deliberate cuts: no semantic-conflict visualization (check-time), full `project_load` re-push only, curve y-axis reuses pitch axis, Stub produces no audio.
23. **History & Collaboration hooks**: Session-level undo/redo — the substrate (`Coconut.Edit.History` op tree, `undo/1` / `redo/1`, version pins) landed with the coconut migration; what remains is LiveView surfacing (undo/redo events, conflict UI for dead patches — dead patches now reach the UI as `patch_conflicts` banners, see 22b; semantic `check_failed` conflicts still only logged); design space for future CRDT.
...
Split into Phase 1 (domain) and Phase 2 (kernel integration). Status: the generic curve channel landed 2026-08-15 (`Port.Channels.Curve` + kernel `CurveRaster` + arity-2 spec target) — `Coconut.Curve.*` stays float-world and digest-free, wrapped by plain-map payloads; RasterCache / Douglas-Peucker / `Curve.Chunk` pickle remain deferred.
...
- [x] Curve channel (`Coconut.Render.Channel` impl for continuous data) — landed 2026-08-15: `Port.Channels.Curve` (generic `:curve` atom; reference implementation pattern follows `Port.Channels.PhonemeTiming`).
...
- [x] Define curve channel(s) and emit curve `data_interventions` (resolve at check time) — landed 2026-08-15: `:curve` spec target (arity-2) rasterizes via `CurveRaster` and emits `%{param, start_tick, end_tick, stride, samples}` to `{:port, :synth, param}`.
