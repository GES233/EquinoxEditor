...
- `Score.TrackMeta` — equinox-only per-track side table: `{mix_automation, gain, pan, mute, solo, voicebank_id, presets, active_preset, ui_state, metadata}`. Lives outside coconut History (not undoable). Own dump/load. `voicebank_id` selects the track's EngineAdapter from the session `engines` registry (per-track granularity).
