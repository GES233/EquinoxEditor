...
- `engine_adapters/` — `:equinox_engine_adapters` (userland engine adapter reference implementations `EquinoxAdapters.*`; path-depends on domain + kernel, nothing depends on it; includes the DiffSinger Python sidecar (`sidecar/`, uv-managed) and real-inference Step)
...
22b. **Intervention UI minimal loop (landed 2026-08-16)**: patches are now UI-visible — presenter projects alive patches to `TrackData.patches` (`%{id, channel, anchor: %{kind, refs?}, payload}`, payload pass-through); kernel `Session.Context.notifications` + `Server.take_notifications/1` is the UI-bound drain for dead patches (`run_history` enqueues `{:dead_patches, [...]}` alongside the log); LiveView pushes `patch_conflicts` events (dead-patch entries + adopt failures); PianoRoll renders `:curve` payloads read-only (`CurveLayer.svelte` SVG polyline over the pitch axis, `curve_projection.ts` pure segment-relative mapping), shows a patch chip list, and the demo button drives real `Server.adopt_intervention/3` (`adopt_demo_curve` event); demo project seeds one pitch curve patch on load. Deliberate cuts: no stroke editing (Douglas-Peucker still deferred), no semantic-conflict visualization (check-time), full `project_load` re-push only.
