...
会重新创建。此阶段没有工程保存 UI、渲染/播放或 Bezier 编辑。

「模型输出编辑」支持先提取、再修改逐音素帧长与预测音高。灰线是预测、控制点
是修改目标；只有点击应用才提交。duration 修改可能使已有 pitch 底料变化，
显示非模态警告，可显式沿用或移除；折叠提示不会接受冲突。示例歌词 `la` 会被
mock 拆为两个演示音素，可试着在保持总帧数的前提下调整边界。
原卷帘音高调校仍是 v2 谱面语义，不自动升级；使用输出型 pin 前须显式移除旧调校。
后端语义与限制见 [模型输出底料](../neume/docs/model-output-pins.md)。
